...
class HomePresenter: NSObject {
    ...
    func switchVPN(completion: @escaping (Error?) -> Void) {
        VPN.switchVPN(group) { [unowned self] (error) in
            if let error = error {
                AppAlert.show(self.vc, message: "\("Fail to switch VPN.".localized()) (\(error))")
                completion(error)
            } else {
                completion(nil)
            }
        ...
        alert.addAction(UIAlertAction(title: "OK".localized(), style: .default, handler: { (action) in
            if let input = urlTextField?.text {
                do {
                    ...
                    AppAlert.show(self.vc, message: "\("Failed to add config group".localized()): \(error)")
                }
            }
        }))
    ...
    func updateDNS(_ dnsString: String) {
        ...
        if trimmedDNSString.count > 0 {
            ...
            if !valid {
                dns = ""
                AppAlert.show(self.vc, title: "Invalid DNS".localized(), message: "DNS should be valid ip addresses (separated by commas if multiple). e.g.: 8.8.8.8,8.8.4.4".localized())
            }else {
        ...
        alert.addAction(UIAlertAction(title: "OK".localized(), style: .default, handler: { [unowned self] (action) in
            if let newName = urlTextField?.text {
                do {
                    ...
                    AppAlert.show(self.vc, title: "Failed to change name", message: "\(error)")
                }
                self.delegate?.handleRefreshUI()
            }
        }))
